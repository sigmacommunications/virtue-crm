                ...
                @method('PUT') {{-- Use PUT method for updating --}}
                 <article>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="user_id" class="form-control">
                                    ...
                                    @foreach ($users as $agent)
                                        <option value="{{ $agent->id }}" @if($agent->id == $invoice->user_id) selected  @endif>{{ $agent->name }}
                                            - {{ $agent->email }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                              ...
                              <select name="company" class="form-control">
                                  ...
                                  @foreach ($companies as $company)
                                      <option value="{{ $company->id }}" @if($company->id == $invoice->company_id) selected  @endif>{{ $company->title }}</option>
                                  @endforeach
                              </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="client_id" class="form-control">
                                    ...
                                    @foreach ($clients as $cl)
                                        <option value="{{ $cl->id }}" @if($cl->id == $invoice->client_id) selected  @endif>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select name="payment_method" class="form-control">
                                    ...
                                    @foreach ($pms as $pm)
                                        <option value="{{ $pm->id }}" @if($pm->id == $invoice->payment_method) selected  @endif>{{ $pm->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <table class="meta">
                                ...
                                <tr>
                                    ...
                                    <td><span
                                            >{{ now()->format('F j, Y') }}</span>
                                    </td>
                                </tr>
                                <tr>
                                    <th><span >Amount Due</span></th>
                                    <td><span id="prefix" >$</span><span
                                            id="amount-due">0.00</span></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                      <table class="inventory" id="invoice-items">
                        ...
                        <tbody>
                          @foreach ($invoice->details as  $id)
                            <tr id="append-row">
                                <td>
                                <select name="product[]" class="form-control" >
                                    @foreach ($products as $pro)
                                    <option value="{{ $pro->id }}" @if($pro->id == $id->product) selected @endif>{{ $pro->name }} </option>
                                    @endforeach
                                </select>
                                </td>
                                <td><input name="description[]" value="{{ $id->description}}" class="form-control" ></input></td>
                                <td><input name="amount[]" value="{{ $id->amount}}" class="form-control"></input></td>
                                <td><input name="qty[]" value="{{ $id->qty}}" class="form-control"></input></td>
                                <td><input name="discount[]" value="{{ $id->discount}}"  class="form-control"></input></td>
                                <td>
                                    <select class="form-control tax-select" name="tax[]">
                                        <option value="">Select a Tax</option>
                                        @foreach ($taxes as $tax)
                                            <option value="{{ $tax->id }}" @if($tax->id == $id->tax) selected @endif>{{ $tax->name  }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td><input name="total_amount[]" value="{{ $id->total_amount}}" class="form-control" readonly></input></td>
                                <td><a class="cut">-</a></td>
                            </tr>
                          @endforeach
                        </tbody>
...
<script>
    $(document).ready(function() {
        ...
        invoiceItems.on('input', '.tax-select ,input[name="amount[]"], input[name="qty[]"] , input[name="discount[]"]', updateTotal);
        invoiceItems.on('change', '.tax-select', updateTotal);
